...
@Composable
fun CompromissoPessoalScreen(
    compromissos: List<CompromissoPessoal>,
    onVoltar: () -> Unit,
    onAdicionar: () -> Unit,
    onEditar: (CompromissoPessoal) -> Unit,
    onExcluir: (CompromissoPessoal) -> Unit,
    modifier: Modifier = Modifier
) {
    ListaGenericaScaffold(
        titulo = "Compromissos pessoais",
        itens = compromissos.sortedBy { it.data },
        chave = { it.id },
        onVoltar = onVoltar,
        onAdicionar = onAdicionar,
        mensagemVazia = "Nenhum compromisso pessoal cadastrado ainda.",
        modifier = modifier
    ) { item ->
        CartaoItemGenerico(
            titulo = item.titulo,
...
@Composable
fun CompromissoPessoalFormScreen(
    compromisso: CompromissoPessoal?,
    onVoltar: () -> Unit,
    onSalvar: (CompromissoPessoal) -> Unit,
    modifier: Modifier = Modifier,
    onExcluir: (() -> Unit)? = null
) {
    ...
    TelaFormularioScaffold(
        ...
        onSalvar = {
            onSalvar(
                (compromisso ?: CompromissoPessoal(titulo = titulo, data = data)).copy(
                    titulo = titulo,
                    data = data,
        ...
        CampoTexto(titulo, { titulo = it }, "Título do compromisso (ex.: Terapia)")
        CampoBooleano(
            valor = recorrente,
